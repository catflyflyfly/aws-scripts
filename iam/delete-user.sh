...
# Credit: https://gist.github.com/hernandanielg
...
do
  ...
  aws iam detach-user-policy --user-name $user --policy-arn $olicy_arn
done
...
user_mfa_serial_numbers=$(aws iam list-mfa-devices --user $user --query 'MFADevices[*].SerialNumber' --output text)

echo "Deleting user mfa devices: $user_mfa_serial_numbers"
for sn in $user_mfa_serial_numbers ;
do
  echo "aws iam deactivate-mfa-device --user $user --serial-number $sn" 
  aws iam deactivate-mfa-device --user $user --serial-number $sn
done
